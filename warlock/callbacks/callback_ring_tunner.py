...
class CallbackRingTunner(Callback):
    """
    ...
    def __init__(
        ...
        Example this a map that describe value for each nic host.
        On each call on_iaas_mutate_begin we update ring size.
    ...
    def sample_value(
        ...
        """On sample, method return values that we mutate on follow-up call
        next etc.  Note this method is cycle back i.e. restart on follow-up calls.
        ...
        {
            'x.x.x.x': [('vmnic0', 1024, 1024), ('vmnic1', 1024, 1024)],
            'y.y.y.y': [('vmnic2', 1024, 1024), ('vmnic3', 1024, 1024)]
        }

        Second scenario need mutate vmnic0/1 on x and 0/1 on y.

        {
            'x.x.x.x': [('vmnic0', 4096, 4096), ('vmnic1', 4096, 4096)],
            'y.y.y.y': [('vmnic2', 4096, 4096), ('vmnic3', 4096, 4096)]
        }
